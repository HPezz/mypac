...
export function buildCommitPrompt(options: CommitPromptOptions): string {
	return [
		...
		"2. Follow skills/pac-commit/SKILL.md for branch safety, gitmoji shortlist selection, atomic grouping, explicit staging, and GitHub issue closing references.",
		...
		"   - If the work is explicitly tied to a GitHub issue, add `closes #<issue>` to the commit body that should close it when merged.",
